...
st.set_page_config(
    page_title="QuLab: Lab 53: Bond Rating Prediction", layout="wide")
...
def initialize_session_state():
    ...
    if 'n_issuers' not in st.session_state:
        st.session_state.n_issuers = 1000  # Fixed value for consistent data
...
if st.session_state.page == "Home":
    ...
    st.markdown(
        f"A seasoned Credit Analyst at \"Alpha Asset Management,\" a fixed income investment firm.")
...
$$""")
    ...
    st.markdown(
        r"Conversely, `debt_to_ebitda` is negatively correlated, meaning higher quality companies tend to have lower debt:")
    st.markdown(r"""
$$
...
$$""")
    ...
    # Fixed number of issuers for consistent data generation
    st.session_state.n_issuers = 1000
    st.info(
        f"Data will be generated for {st.session_state.n_issuers} issuers (fixed for consistency)")

    if st.button("Generate Corporate Data"):
        st.session_state.corp_data, st.session_state.financial_features, st.session_state.latent_quality = generate_corporate_data(
            st.session_state.n_issuers)
    ...
    st.markdown(
        r"The feature vector for each issuer $i$ is: $x_i = [ X_{i,1}, ..., X_{i,8}, S_{i,1}, S_{i,2}, S_{i,3} ]$")
    st.markdown(
        r"where $X_{i,j}$ are the financial ratios and $S_{i,k}$ are the NLP sentiment features.")
    st.markdown(
        r"-   **Financial ratios** ($X_{i,1}, ..., X_{i,8}$) capture the *current* financial position: leverage (`debt/EBITDA`), coverage (`interest coverage`), profitability (`margins`), liquidity (`current ratio`), and size (`total assets`).")
    ...
    if st.button("Add NLP Sentiment Features"):
        if st.session_state.corp_data is not None:
            st.session_state.corp_data, st.session_state.nlp_features = add_sentiment_features(
                st.session_state.corp_data, st.session_state.latent_quality, st.session_state.n_issuers)
            # Update all features list
            st.session_state.all_features = st.session_state.financial_features + \
...
# --- PAGE 2: MODEL TRAINING ---
    ...
    st.markdown(
        r"Given predicted numerical rating $\hat{y}$ and actual numerical rating $y$, the 'within-1-notch accuracy' is defined as the proportion of predictions where the absolute difference between the predicted and actual rating is less than or equal to 1:")
    st.markdown(r"""
$$
            ...
            if os.path.exists(model_path):
                ...
                with st.spinner("Loading saved models..."):
                    try:
                        ...
                        model_full = saved_data['model_full']
                        model_fin_only = saved_data['model_fin_only']
                        X_test = saved_data['X_test']
                        y_test = saved_data['y_test']
                        test_idx = saved_data['test_idx']
                        pred_full_numeric = saved_data['pred_full_numeric']
                        pred_fin_numeric = saved_data['pred_fin_numeric']
                        ...
                        within1_fin = np.mean(
                            np.abs(pred_fin_numeric - y_test) <= 1)
                        within1_full = np.mean(
                            np.abs(pred_full_numeric - y_test) <= 1)

                        print("\nRATING PREDICTION PERFORMANCE")
                        print("=" * 55)
                        print(
                            f"{'Metric':<30s}{'Financials Only':>15s}{'+ NLP Sentiment':>15s}")
                        print(
                            f"{'Exact accuracy':<30s}{acc_fin:>15.1%}{acc_full:>15.1%}")
                        print(
                            f"{'Within 1-notch accuracy':<30s}{within1_fin:>15.1%}{within1_full:>15.1%}")
                        nlp_contribution = acc_full - acc_fin
                        print(
                            f"{'NLP contribution':<30s}{'---':>15s}{nlp_contribution:>+15.1%}")

                        # Plot Confusion Matrix
                        plt.figure(figsize=(8, 6))
                        cm = confusion_matrix(y_test, pred_full_numeric)
                        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                                    xticklabels=[RATING_MAP[i]
                                                 for i in sorted(RATING_MAP.keys())],
                        ...
                        st.pyplot(plt.gcf())
                        st.success("Models loaded from saved file.")
                    except Exception as e:
                        st.error(
                            f"Error loading saved models: {e}. Training new models...")
                        # Fall through to training
                        train_new_model = True
            else:
                train_new_model = True

            # Train new models if no saved models or loading failed
            if not os.path.exists(model_path) or 'train_new_model' in locals():
                with st.spinner("Training models (this may take a while)..."):
                    ...
                    with open(model_path, 'wb') as f:
                        pickle.dump({
                            ...
                            'model_fin_only': model_fin_only,
                            'X_test': X_test,
                            'y_test': y_test,
                            'test_idx': test_idx,
                            'pred_full_numeric': pred_full_numeric,
                            'pred_fin_numeric': pred_fin_numeric
                        }, f)
...
# --- PAGE 3: MISMATCH ANALYSIS ---
    ...
    st.header(
        "3. Identifying Opportunities & Risks: Implied vs. Actual Rating Mismatches")
    ...
    st.markdown(
        r"For each issuer, the `notch_diff` is calculated as the numerical implied rating minus the numerical actual rating:")
    st.markdown(r"""
$$
...
$$""")
    ...
    st.markdown(
        r"-   **DOWNGRADE RISK**: $\text{notch\_diff} \le -2$ (model predicts at least 2 notches lower)")
    st.markdown(
        r"-   **UPGRADE CANDIDATE**: $\text{notch\_diff} \ge 2$ (model predicts at least 2 notches higher)")
    st.markdown(r"-   **WATCH (1 notch)**: $|\text{notch\_diff}| = 1$")
    st.markdown(r"-   **ALIGNED**: $\text{notch\_diff} = 0$")
...
# --- PAGE 4: EXPLAINABLE AI ---
    ...
    if st.session_state.mismatches_df is None:
        st.warning(
            "Please perform Mismatch Analysis (Step 3) first to identify issuers for explanation.")
    else:
        categories = sorted(
            st.session_state.mismatches_df['signal'].unique())
        selected_category = st.selectbox(
            "Select Mismatch Category", categories)

        issuers_in_cat = st.session_state.mismatches_df[st.session_state.mismatches_df[
            'signal'] == selected_category]['issuer_id'].unique()
        ...
        if st.button(f"Explain {selected_issuer_id}'s Rating"):
            ...
            if st.session_state.explainer_full is None:
                st.session_state.explainer_full = shap.TreeExplainer(
                    st.session_state.model_full)
            ...
            row_idx = st.session_state.corp_data[st.session_state.corp_data['issuer_id']
                                                 == selected_issuer_id].index[0]
            ...
            if row_idx in st.session_state.test_idx:
                ...
                issuer_features = st.session_state.corp_data.loc[[
                    row_idx], st.session_state.all_features].values[0]
                actual_numeric = st.session_state.corp_data.loc[row_idx, 'rating']

                # Predict again to get implied numeric
                implied_numeric = st.session_state.model_full.predict(issuer_features.reshape(1, -1))[
                    0]
...
# --- PAGE 5: NLP CONTRIBUTION & WATCHLIST ---
    ...
    st.header(
        "5. Quantifying NLP's Edge & Prioritizing Action: Watchlist & Contribution Analysis")
    ...
    if st.button("Perform NLP Contribution Analysis"):
        if st.session_state.pred_full_numeric is not None and st.session_state.y_test is not None:
            ...
            diff_full = np.abs(
                st.session_state.pred_full_numeric - st.session_state.y_test)
            diff_fin = np.abs(
                st.session_state.pred_fin_numeric - st.session_state.y_test)
    ...
    if st.button("Build Downgrade Watchlist"):
        if st.session_state.mismatches_df is not None:
            downgrade_watchlist = build_watchlist(
                st.session_state.mismatches_df, notch_threshold=-2, top_n=10)
